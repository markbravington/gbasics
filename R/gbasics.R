...
This is probably the _wrong_ place to describe the various encodings currently used by 'kinference' (a few others are or were used by CSIRO's own genotyping pipeline before the results are 'kinference'-ready). Anyway, the commonest ones are:
 ...
 - 6-way ('genotypes6') with possibilities "AA", "AB", "AO", "BB", "BO", and "OO". Here, the genotyping process has tried to distinguish between single-nulls and true homozygotes, based on read-depth. The discrimination will never be 100% accurate, even for "good" loci, so explicit allowance must be made for getting it wrong sometimes (if you see references to 'snerr', that's why).  For some loci it's not even worth trying, so for them 'locinfo$useN' is set to 4 or 3 (and it is set to 6 for loci where 6-way is really being used). The pipeline for 6-way genotyping uses in-house CSIRO code and is quite fiddly--- so it's likely to _stay_ in-house! CSIRO has used 6-way genotyping for _large_ ongoing studies of SBTuna and school sharks, so this encoding will continue to be supported, but for new projects we would recommend 4-way instead, using more loci to compensate.
...
Mostly, encoding should be handled automatically during the creation of your 'snpgeno'. However, you do sometimes need do access to it, and _occasionally_ you might want to manually recode your data. That takes some care because the internal representation of the encodings won't generally match up; for example, "AB" is encoded as 1 for 'genotypes3' but as 2 in 'genotypes4_ambig'. You can use 'get_genotype_encoding' to change the encoding, then manually change old genotypes to new ones. Here's an example that changes all genotypes from 3-way to 4-way (which you also could do less painfully via 'useN'--- hopefully that is documented somewhere).
...
'define_genotypes' is meant only for use inside other functions, and to help explain the flexible encoding system for genotype storage used by 'snpgeno'.  Once you've called it inside a function, you can refer directly to a genotype '"AB"' as just 'AB' etc (ie no quotes), and you can also refer to sets of genotype-encoding that are commonly used in 'snpgeno' objects; those sets are just character vectors. It creates objects such as 'AB' *in its caller*, so a lot of kinference functions run 'define_genotypes' internally as a first step to simplify their subsequent code- and that is the intended use. 
